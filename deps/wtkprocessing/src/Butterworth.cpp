...
Butterworth::prototypeAnalogLowPass(int filterOrder){
    ...
    for(auto k = 0; k < (filterOrder + 1) / 2; k++){
...
bool Butterworth::coefficients(FILTER_TYPE filter, const double fs, const double freq1_cutoff,
    ...
    gain = 1.0;         // always 1 for the butterworth prototype lowpass. 1.0
...
bool Butterworth::zp2SOS(){
    ...
    for(auto i = numZeros; i < filterOrder; i++){
    ...
    int numSOS = 0;
    for(auto i = 0; i + 1 < filterOrder; i += 2, numSOS++){
...
void Butterworth::convert2lopass(){
    ...
    for(auto i = 0; i < numPoles; i++){    // scale poles by the cutoff Wc
        poles[i] = Wc * poles[i];
    }
}
...
void Butterworth::convert2bandpass(){
    ...
    for(auto i = 0; i < numPoles; i++){    // First set of poles + conjugates
        if(abs(poles[i])){
    ...
    for(auto i = 0; i < numPoles; i++){    // Second set of poles + conjugates
        if(abs(poles[i])){
            complex_double firstterm = 0.5 * poles[i] * bw;
            complex_double secondterm = 0.5 * sqrt((bw * bw) * (poles[i] * poles[i]) - 4 * Wc * Wc);
