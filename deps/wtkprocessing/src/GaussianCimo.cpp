...
#include "GaussianCimo.hpp"

namespace wtk {
    namespace proc {

GaussianCimo::GaussianCimo(void) : Classifier(Classifier::AsGaussian) {
...
GaussianCimo::~GaussianCimo(void) {
}


bool GaussianCimo::Setup(std::string filename) {
...
bool GaussianCimo::Setup(gauconfig_t* config, const Eigen::Ref<const Eigen::MatrixXd>& centers, const Eigen::Ref<const Eigen::MatrixXd>& covs) {
...
bool GaussianCimo::Export(std::string filename, std::ios::openmode mode) {
...
void GaussianCimo::Run(const Eigen::Ref<const Eigen::VectorXd>& in, Eigen::Ref<Eigen::VectorXd> out) {
