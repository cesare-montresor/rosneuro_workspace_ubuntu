...
void BiquadChain::processBiquad(const Eigen::Ref<const Eigen::VectorXd> &in, Eigen::Ref<Eigen::VectorXd> out, const int stride, const int count, const vector <Biquad> coeffs){


    //std::cout <<"I'm doing biquad" << std::endl;
    for(int n = 0; n < count; n=n+stride)
    {
        double xn = in[n];

        _yn[0] = coeffs[0].b0 * xn + coeffs[0].b1 * _xn1 + coeffs[0].b2 * _xn2
        + coeffs[0].a1 * _yn1[0] + coeffs[0].a2 * _yn2[0];

        for(int i = 1; i < numFilters; i++){
            _yn[i] = coeffs[i].b0 * _yn[i - 1] + coeffs[i].b1 * _yn1[i - 1] + coeffs[i].b2 * _yn2[i - 1]
            + coeffs[i].a1 * _yn1[i] + coeffs[i].a2 * _yn2[i];
        }
...
void BiquadChain::processBiquadSG(const Eigen::Ref<const Eigen::VectorXd> &in, Eigen::Ref<Eigen::VectorXd> out, const int stride, const int count, const vector <Biquad> coeffs){

    //std::cout<< "Debug biquad" << std::endl;
    for(int n = 0; n < count; n++){ //nsamples
        double xn = in[n];

        _yn[0] = coeffs[0].b0 * xn + coeffs[0].b1 * _xn1 + coeffs[0].b2 * _xn2
                + coeffs[0].a1 * _yn1[0] + coeffs[0].a2 * _yn2[0];

        for(int i = 1; i < numFilters; i++){ //nfilter
            _yn[i] = coeffs[i].b0 * _yn[i - 1] + coeffs[i].b1 * _yn1[i - 1] + coeffs[i].b2 * _yn2[i - 1]
                    + coeffs[i].a1 * _yn1[i] + coeffs[i].a2 * _yn2[i];
        }
...
void BiquadChain::processFourthOrderSections(const float * input, float * output, const int stride, const int count, const Biquad * coeffs){ //to test
